...
useEffect(()=>{
  ...
  .then(res =>{
    ...
    let renderProducts = productData.map((item) => <ProductCard key={item._id} productId={item._id} productName={item.productName} productPrice={item.productPrice} discount={item.productDiscount} editRender={setUpdateProducts} image={item.image}/>)
